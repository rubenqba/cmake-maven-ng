...
public class TestMojo extends AbstractMojo {
	 ...
	 * Name of this test.
	 * 
	 * Defaults to the basename of the binary. So if your binary is
	 * /foo/bar/baz, this will default to 'baz.'
	 * 
	 ...
	 * 
	 * @parameter expression="${expectedResult}" default-value="success" Can be
	 *            either "success", "failure", or "any".
	 */
	...
	private void writeStatusFile(String status) throws IOException {
		FileOutputStream fos = new FileOutputStream(new File(results, testName
		        + ".status"));
	...
	final private String VALID_PRECONDITION_TYPES_STR = "Valid precondition types are \"and\", \"andNot\"";
	...
	private void validateParameters() throws MojoExecutionException {
		if (!(expectedResult.equals("success")
		        || expectedResult.equals("failure") || expectedResult
		            .equals("any"))) {
	...
	private boolean shouldRunTest() throws MojoExecutionException {
		...
		String skipTests = session.getExecutionProperties().getProperty(
		        "skipTests");
		...
		if (testProp != null) {
			...
			if (!found) {
				getLog().info(
				        "did not find test '" + testName + "' in " + "list "
				                + testProp);
				return false;
			}
		}
			...
			for (Map.Entry<String, String> entry : preconditions.entrySet()) {
				...
				if (key == null) {
					throw new MojoExecutionException("NULL is not a valid "
					        + "precondition type.  "
					        + VALID_PRECONDITION_TYPES_STR);
				}
				if (key.equals("and")) {
					if (!isTruthy(val)) {
						getLog().info(
						        "Skipping test " + testName
						                + " because precondition number " + idx
						                + " was not met.");
						return false;
					}
	...
	public void execute() throws MojoExecutionException {
		...
		try {
			proc = pb.start();
			errThread = new OutputToFileThread(proc.getErrorStream(), new File(
			        results, testName + ".stderr"));
			errThread.start();
			// Process#getInputStream gets the stdout stream of the process,
			// which
			// acts as an input to us.
			outThread = new OutputToFileThread(proc.getInputStream(), new File(
			        results, testName + ".stdout"));
			outThread.start();
			...
			throw new MojoExecutionException(
			        "IOException while executing the test " + testName, e);
		} catch (InterruptedException e) {
			throw new MojoExecutionException("Interrupted while executing "
			        + "the test " + testName, e);
		} finally {
			System.out.println("testThread = " + testThread + ", retCode = "
			        + retCode + ", proc = " + proc + ", errThread = "
			        + errThread + ", outThread = " + outThread);
			if (testThread != null) {
				// If the test thread didn't exit yet, that means the timeout
				// expired.
				...
				try {
					...
					System.err
					        .println("Interrupted while waiting for testThread");
			...
			if (errThread != null) {
				try {
					...
					getLog().error("Interrupted while waiting for errThread", e);
				}
				errThread.close();
			}
			if (outThread != null) {
				try {
					...
					getLog().error("Interrupted while waiting for outThread", e);
				}
				outThread.close();
			}
		}
